...
DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_CIRCLE, DP_TYPE_VALUE},
  {DPID_FAN, DP_TYPE_BOOL},
  {DPID_FAN_SPEED, DP_TYPE_VALUE},
  {DPID_WATER_PRESS, DP_TYPE_VALUE},
  {DPID_TEM_UP, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
  ...
  extern int turns;
  extern int TEM, HUM;
  extern int bright_number;
  extern int tem_up_a;
  extern int water_press_a;
  extern int bright_number;
  extern int fan_number;
  bool hot, water, led, fan;
  ...
  mcu_dp_bool_update(DPID_SWITCH, hot); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_PUMP, water); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_LIGHT, led); //BOOL型数据上报;
  mcu_dp_value_update(DPID_BRIGHT_VALUE, bright_number); //VALUE型数据上报;
  mcu_dp_value_update(DPID_TEMP_CURRENT, TEM); //VALUE型数据上报;
  mcu_dp_value_update(DPID_HUMIDITY_CURRENT, HUM); //VALUE型数据上报;
  mcu_dp_value_update(DPID_CIRCLE, turns); //VALUE型数据上报;
  mcu_dp_bool_update(DPID_FAN, fan); //BOOL型数据上报;
  mcu_dp_value_update(DPID_FAN_SPEED, fan_number); //VALUE型数据上报;
  mcu_dp_value_update(DPID_WATER_PRESS, water_press_a); //VALUE型数据上报;
  mcu_dp_value_update(DPID_TEM_UP, tem_up_a); //VALUE型数据上报;
...
static unsigned char dp_download_pump_handle(const unsigned char value[], unsigned short length)
{
  ...
  if (pump == 0) {
    ...
    //开关开
    analogWrite(9, map(water_press_a, 0, 10, 0, 254));
  }
...
static unsigned char dp_download_light_handle(const unsigned char value[], unsigned short length)
{
  ...
  if (light == 0) {
    ...
    //开关开
    analogWrite(5, map(bright_number, 0, 10, 0, 254));
  }
...
static unsigned char dp_download_bright_value_handle(const unsigned char value[], unsigned short length)
{
  ...
  bright_number = bright_value;
  EEPROM.write(40, bright_number);
  analogWrite(5, map(bright_number, 0, 10, 0, 254));
...
static unsigned char dp_download_fan_handle(const unsigned char value[], unsigned short length)
{
  ...
  if (fan == 0) {
    ...
    analogWrite(6, map(fan_number, 0, 10, 0, 254));
    //开关开
  }
...
/*****************************************************************************
  函数名称 : dp_download_fan_speed_handle
  功能描述 : 针对DPID_FAN_SPEED的处理函数
  输入参数 : value:数据源数据
        : length:数据长度
  返回参数 : 成功返回:SUCCESS/失败返回:ERROR
  使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
static unsigned char dp_download_fan_speed_handle(const unsigned char value[], unsigned short length)
{
  ...
  fan_number = fan_speed;
  EEPROM.write(30, fan_number);
  analogWrite(6, map(fan_number, 0, 10, 0, 254));
  mcu_dp_bool_update(DPID_FAN, 1);
  /*
    //VALUE类型数据处理
...
/*****************************************************************************
  函数名称 : dp_download_water_press_handle
  功能描述 : 针对DPID_WATER_PRESS的处理函数
  输入参数 : value:数据源数据
        : length:数据长度
  返回参数 : 成功返回:SUCCESS/失败返回:ERROR
  使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_water_press_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为VALUE
  unsigned char ret;
  unsigned long water_press;
  extern int water_press_a;

  water_press = mcu_get_dp_download_value(value, length);
  water_press_a = water_press;
  EEPROM.write(20, water_press_a);
  analogWrite(9, map(water_press_a, 0, 10, 0, 254));
  mcu_dp_bool_update(DPID_PUMP, 1);
  ...
  */
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_value_update(DPID_WATER_PRESS, water_press);
  if (ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
  函数名称 : dp_download_tem_up_handle
  功能描述 : 针对DPID_TEM_UP的处理函数
  输入参数 : value:数据源数据
        : length:数据长度
  返回参数 : 成功返回:SUCCESS/失败返回:ERROR
  使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_tem_up_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为VALUE
  unsigned char ret;
  unsigned long tem_up;
  extern int tem_up_a;

  tem_up = mcu_get_dp_download_value(value, length);
  tem_up_a = tem_up;
  EEPROM.write(10, tem_up_a);
  ...
  */
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_value_update(DPID_TEM_UP, tem_up);
  if (ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
  ...
  ***********************************/
  ...
  switch (dpid) {
    ...
    case DPID_FAN_SPEED:
      ...
      break;
    case DPID_WATER_PRESS:
      //水泵速度处理函数
      ret = dp_download_water_press_handle(value, length);
      break;
    case DPID_TEM_UP:
      //控温上限处理函数
      ret = dp_download_tem_up_handle(value, length);
      break;
...
//#error "请自行完成相关代码,并删除该行"
  ...
  */
  ...
  if (time[0] == 1) {
    //正确接收到wifi模块返回的格林数据
    if (time[4] == 4 && time[5] == 0 ) {//每天中午12点清空跑轮圈数
turns = 0;
mcu_dp_value_update(DPID_CIRCLE, turns); }//VALUE型数据上报;
  } else {
